...
our ( $ra, $total_lifts, $match, $tmp, @hash_match, %regex, %hoh );                                                    # Global var
our $epoc = time();
...
sub build_hash {                                                                                                       # Find first$last BCR3 timestamp & total lifts for batch & first timestamp next batch
  my ( $match, $total_batches, $string, $timestamp, $id, $batchid_next );
  my $batchid = "";
  foreach (@hash_match) {
    if ( $_ =~ qr((^([-\d:.\s]+)(?=:)[^<]+<14[^X#]+[X#].{6}(.{5}).+15=(\d+)\)$)) ) {
      ...
      unless ( $batchid_next =~ qr(^[^_\W]{5}$) ) {
        find_batchid_bcr2( \$batchid_next, $id );
      }                                                                                                                # If BCR3 noread update batchid from BCR2
      if ( $batchid !~ $batchid_next ) {
        $hoh{"BatchId"}{$batchid}{"FirstNextBatch"} = $timestamp;                                                      # First Event in next batch
        $hoh{"BatchId"}{$batchid}{"ID"}             = $id;
        $batchid                                    = $batchid_next;
        $match                                      = 1;
        $hoh{"BatchId"}{$batchid}{"First"}          = $timestamp;                                                      # First Event in batch
        $hoh{"BatchId"}{$batchid}{"Last"}           = $timestamp;                                                      # Last Event in batch
        $hoh{"BatchId"}{$batchid}{"Lifts"}          = $match;
      }
      else {
        $match++;
        $hoh{"BatchId"}{$batchid}{"Last"} = $timestamp;                                                                # Last Event in batch
  ...
  $hoh{"BatchId"}{"Total Lifts"}   = $total_lifts;
  $hoh{"BatchId"}{"Total Batches"} = batchid_count();
}
...
sub timediff {                                                                                                         # Calculate timediff between first $ last lift in batch,
  ...
  foreach my $batchid ( sort keys %hoh ) {
    if ( $batchid =~ qr(^[^_\W]{5}$) ) {
      my $sts1 = $hoh{"BatchId"}{$batchid}{"First"};
      my $ets1 = $hoh{"BatchId"}{$batchid}{"Last"};
      ...
      $hoh{"BatchId"}{$batchid}{"Diff1"} = $diff1;

      if ( $hoh{"BatchId"}{$batchid}{"FirstNextBatch"} ) {
        my $sts2  = $hoh{"BatchId"}{$batchid}{"Last"};
        my $ets2  = $hoh{"BatchId"}{$batchid}{"FirstNextBatch"};
        ...
        $hoh{"BatchId"}{$batchid}{"Diff2"} = $diff2;
      }
    }
  }
}

sub find_shid {                                                                                                        # Find ShuttleId matching to first lift next batch and update %hoh
  foreach my $string (@hash_match) {
    foreach my $batchid ( keys %hoh ) {
      if ( $hoh{"BatchId"}{$batchid}{"ID"} ) {
        if (
           $string =~ qr((^([-\d:.\s]+)(?=:)[^<]+<13[^\(]+\(ShuttleId=(\d+).+15=($hoh{"BatchId"}{$batchid}{"ID"})\)$)) )
        {
          $hoh{"BatchId"}{$batchid}{"SHID"} = $4;
        }
      }
    }
  }
...
sub batchid_count {
  my $batchid_count_ref;
  foreach my $key ( keys %hoh ) {
    foreach my $batchid ( keys %{ $hoh{$key} } ) {
      if ( $batchid =~ qr(^[^_\W]{5}$) ) { $batchid_count_ref++ }
    }
  }
  return $batchid_count_ref;
}

sub find_events {                                                                                                      # Find all events corresponding to first lift next batch
  foreach my $string (@hash_match) {
    foreach my $batchid ( keys %hoh ) {
      if ( defined( $hoh{"BatchId"}{$batchid}{"SHID"} ) ) {
        if ( $string =~ qr(<404.+$hoh{"BatchId"}{$batchid}{"SHID"}) ) {
          $hoh{"BatchId"}{$batchid}{"Event_TrolleyEnteredJunction"} = $string;
          next;
        }
        if ( $string =~ qr(<401.+$hoh{"BatchId"}{$batchid}{"SHID"}) ) {
          $hoh{"BatchId"}{$batchid}{"Event_TrolleyLeftJunction"} = $string;
          next;
        }
        if ( $string =~ qr(<8.+$hoh{"BatchId"}{$batchid}{"SHID"}) ) {
          $hoh{"BatchId"}{$batchid}{"Event_TrolleyEnteredWorkspace"} = $string;
          next;
        }
        if ( $string =~ qr(<13.+$hoh{"BatchId"}{$batchid}{"SHID"}) ) {
          $hoh{"BatchId"}{$batchid}{"Event_LiftUnloaded"} = $string;
          next;
        }
        if ( $string =~ qr(<14.+$hoh{"BatchId"}{$batchid}{"ID"}) ) {
          $hoh{"BatchId"}{$batchid}{"Event_LiftDetectedAtBcr3"} = $string;
        }
      }
    }
  }
...
sub report {                                                                                                           # Write report if timediff between last lift and
  ...
  foreach my $batchid ( sort keys %hash ) {
    $total_batches++;
    if ( defined($batchid) ) {
      if ( defined( $hash{"BatchId"}{$batchid}{"Diff2"} ) ) {
        if ( $hash{"BatchId"}{$batchid}{"Diff2"} >= $max_time_diff ) {
          my $d1 = $hash{"BatchId"}{$batchid}{"Diff1"};
          my $d2 = $hash{"BatchId"}{$batchid}{"Diff2"};
          my $t1 = format_time( $d1, 1 );
          my $t2 = format_time( $d2, 2 );
          printf "\nBatchId%22s\n",             $batchid;
          printf "Total lifts in batch\t%-s\n", $hash{"BatchId"}{$batchid}{"Lifts"};
          say $t1;
          say $t2;
          printf OUT_F "\nBatchId%22s\n",               $batchid;
...
sub csv {                                                                                                              # Write csv file for all events
  ...
  foreach my $batchid ( keys %hoh ) {
    if ( $batchid ne "0" ) {
      my $row;
      my $first_lift            = $hoh{"BatchId"}{$batchid}{"First"};
      my $last_lift             = $hoh{"BatchId"}{$batchid}{"Last"};
      my $lifts                 = $hoh{"BatchId"}{$batchid}{"Lifts"};
      my $batchtime             = $hoh{"BatchId"}{$batchid}{"Diff1"};
      my $time_to_next_batch    = $hoh{"BatchId"}{$batchid}{"Diff2"};
      my $first_id_next_batch   = $hoh{"BatchId"}{$batchid}{"ID"};
      my $first_shid_next_batch = $hoh{"BatchId"}{$batchid}{"SHID"};
      my $entered_jun           = $hoh{"BatchId"}{$batchid}{"Event_TrolleyEnteredJunction"};
      my $left_jun              = $hoh{"BatchId"}{$batchid}{"Event_TrolleyLeftJunction"};
      my $entered_workspace     = $hoh{"BatchId"}{$batchid}{"Event_TrolleyEnteredWorkspace"};
      my $unloaded              = $hoh{"BatchId"}{$batchid}{"Event_LiftUnloaded"};
      my $bcr3                  = $hoh{"BatchId"}{$batchid}{"Event_LiftDetectedAtBcr3"};
